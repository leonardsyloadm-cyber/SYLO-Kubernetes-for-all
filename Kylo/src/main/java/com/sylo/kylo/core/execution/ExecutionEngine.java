...
public class ExecutionEngine {
    ...
    public java.util.List<Object[]> scanTable(String tableName) {
        ...
        Schema schema = catalog.getTableSchema(tableName);
        if (schema == null)
            return results; // Table not in catalog

        // We need to iterate efficiently.
        // For now, let's just assume we start at Page 0 and follow nextPageId if
        // implemented,
        // or just read known pages if we tracked them.
        // Since HeapFile.java didn't fully implement a page directory, let's try
        // reading Page 0.

        // LIMITATION: This implementation only assumes 1 page or linked pages if
        // nextPageId works.
        // We will improve this by simply iterating page 0. If you added more pages,
        ...
        while (true) {
            ...
            try {
                // If invalid, stop
                if (currentPageId == com.sylo.kylo.core.storage.StorageConstants.INVALID_PAGE_ID)
                    break;
            ...
            if (next == currentPageId || next == 0
                    || next == com.sylo.kylo.core.storage.StorageConstants.INVALID_PAGE_ID) {
    ...
    public int deleteTuple(String tableName, java.util.function.Predicate<Tuple> predicate) {
        int deletedCount = 0;
        Catalog catalog = Catalog.getInstance();
        Schema schema = catalog.getTableSchema(tableName);
        if (schema == null)
    ...
    public void updateTuple(String tableName, Object[] newValues, java.util.function.Predicate<Tuple> predicate) {
