...
// API GET DETALLES (ACTUALIZADA CON TODOS LOS CAMPOS)
...
// DATOS DASHBOARD
$total_revenue = $conn->query("SELECT COALESCE(SUM(p.price), 0) FROM orders o JOIN plans p ON o.plan_id = p.id WHERE o.status='active'")->fetchColumn();
$active_clusters = $conn->query("SELECT COUNT(*) FROM orders WHERE status='active'")->fetchColumn();
...
<head>
    ...
    <style>
        ...
        .kpi-card { background-color: #16213e; border: 1px solid #0f3460; border-radius: 10px; padding: 20px; }
        ...
        /* COLORES PLANES */
        .plan-bronce { color: #cd7f32 !important; text-transform: uppercase; letter-spacing: 1px; }
        .plan-plata { color: #e0e0e0 !important; text-shadow: 0 0 5px rgba(255,255,255,0.3); text-transform: uppercase; letter-spacing: 1px; }
        .plan-oro { color: #ffd700 !important; text-shadow: 0 0 8px rgba(255, 215, 0, 0.6); text-transform: uppercase; letter-spacing: 1px; }
        .plan-custom { color: #3b82f6 !important; text-transform: uppercase; letter-spacing: 1px; font-weight: 700; }

        /* PRECIOS */
        .price-low { color: #fff9c4; font-weight: bold; } 
        .price-mid { color: #ffeb3b; font-weight: bold; } 
        .price-high { color: #ffd600; font-weight: 800; text-shadow: 0 0 10px #ffd600; font-size: 1.1rem; } 
        ...
        .status-active { background-color: #10b981; color: white; box-shadow: 0 0 10px rgba(16, 185, 129, 0.4); }
        ...
        .btn-eliminar:hover { background-color: #b91c1c; color: #fff; box-shadow: 0 0 10px rgba(220, 38, 38, 0.5); }
...
<div class="container-fluid px-5">
    <div class="row g-4 mb-5">
        <div class="col-md-3"><div class="kpi-card"><div class="kpi-value"><?php echo number_format($total_revenue, 2); ?>€</div><small>Ingresos</small></div></div>
        <div class="col-md-3"><div class="kpi-card"><div class="kpi-value"><?php echo $active_clusters; ?></div><small>Activos</small></div></div>
        <div class="col-md-3"><div class="kpi-card"><div class="kpi-value"><?php echo $total_users; ?></div><small>Clientes</small></div></div>
        ...
        <table class="table table-custom align-middle">
            ...
            <tbody>
                <?php foreach($orders as $o): 
                    ...
                    elseif($st == 'error' || $st == 'terminated') $statusClass = 'status-error';
                ?>
                <tr>
...
<script>
    ...
    async function verDetalles(id) {
        ...
        let html = `
            <div class="text-center mb-3">
                <span class="badge bg-light text-dark fs-6">${tipoIcon} ${tipoLabel}</span>
            </div>
            ...
            <div class="detail-row"><span class="detail-label">Usuario:</span> <span class="detail-value">${d.username}</span></div>
            <div class="detail-row"><span class="detail-label">Email:</span> <span class="detail-value text-info">${d.email}</span></div>
        ...
        if (d.tipo_usuario === 'autonomo') {
            ...
            html += `
                ...
                <div class="detail-row"><span class="detail-label">Tipo Soc.:</span> <span class="detail-value">${d.tipo_empresa || 'N/A'}</span></div>
        ...
        html += `
            ...
            <div class="detail-row"><span class="detail-label">Plan:</span> <span class="detail-value badge bg-primary">${d.plan_name}</span></div>
            <div class="detail-row"><span class="detail-label">Fecha Alta:</span> <span class="detail-value">${d.purchase_date}</span></div>
            <div class="detail-row"><span class="detail-label">Estado:</span> <span class="detail-value text-uppercase">${d.status}</span></div>
        `;
    ...
    async function nuke(id) {
        if(!confirm("¿CONFIRMAS ELIMINAR ESTE CLÚSTER?")) return;
        await fetch('admin.php',{method:'POST',body:JSON.stringify({action:'nuke_cluster',order_id:id})});
        location.reload();
    }

    async function purgeAll() {
        if(!confirm("⚠️ ¿PURGA TOTAL DEL SISTEMA?\nSe borrarán TODOS los datos y clústeres.")) return;
